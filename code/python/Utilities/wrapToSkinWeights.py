...
def nmpExtractWeights(cluster, joint, skinCluster, geoName):
    ...
    for i in range(0, len(deformPos)):
        difference = [deformPos[i][0] - originalPos[i][0], deformPos[i][1] - originalPos[i][1],
...
def nmpWrapToJoints():
    ...
    amount = 0
    cmds.progressWindow(title='Transferring Wrap Weights', progress=amount, status='Retrieving Joint Weights',
    ...
    for i, geoName in enumerate(geoNames):
        ...
        # Get Weights por cada Joint - Return Value Per Cluster = [ [joint, distance], [joint, distance], ....... ]
        ...
        for i, clusterHan in enumerate(clusterHandles):
            ...
            cmds.progressWindow(edit=True, progress=amount,
                                status=('Calculating Wrap Weights : %s...' % geoName + `amount` + '%'))

        # ----------------------------------------------------------------------------------------

        # Crear una lista de listas de los Verts weights --- [ [transform, value] ]  --- Reduces Amount of skinPercent Calls
        vertWeights = []
        verts = cmds.ls(geoName + '.vtx[*]', flatten=True)
        ...
        for i, vert in enumerate(verts):
            ...
            amount = int(str(i / float(length) * 100).split('.')[0])
            cmds.progressWindow(edit=True, progress=amount,
                                status=('Assigning Weights: ' + geoName + '  ' + `amount` + '%'))
